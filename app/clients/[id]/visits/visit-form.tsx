...
export type VisitFormDefaults = {
  ...
  nextCheck?: string;
  nextRequired?: string;
  requiredVisitInterval?: string;
  healthHappinessScore?: string;
  rank?: string;
  isManualReturnFlag?: boolean;
  testimonialObtained?: boolean;
  testimonialObtainedDate?: string;
...
export function VisitForm({
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-xl font-semibold text-stone-900 mb-1">{heading}</h1>
      <p className="text-sm text-stone-500 mb-6">{clientName} 様</p>
      ...
      <form action={action} className="flex flex-col gap-5">
        <div className="grid grid-cols-2 gap-4">
          <Field label="来院日" required>
            <input
              ...
              required
              className="input"
              defaultValue={defaults.visitDate ?? new Date().toISOString().slice(0, 10)}
            />
          </Field>
          <Field label="担当スタッフ" required>
            <select name="staffId" required className="input" defaultValue={defaults.staffId ?? ""}>
        ...
        <Field label="本日のメニュー">
          <VisitMenuBuilder
            ...
            defaultMenuPlan={defaults.menuPlan ?? ""}
            defaultTags={defaults.chiefComplaintTags ?? []}
            defaultModalities={defaults.treatmentModalities ?? []}
            defaultOverride={defaults.menuOverride ?? ""}
          />
        </Field>
        ...
        <Field label="前回からの変化">
          <textarea name="changeFromLast" rows={2} className="input" defaultValue={defaults.changeFromLast} />
        </Field>

        <Field label="お客様の声(自己認識)">
          <textarea name="clientVoice" rows={2} className="input" defaultValue={defaults.clientVoice} />
        </Field>

        <div className="grid grid-cols-2 gap-4">
          <Field label="次回確認">
            <input name="nextCheck" className="input" defaultValue={defaults.nextCheck} />
          </Field>
          <Field label="次回必須">
            <input name="nextRequired" className="input" defaultValue={defaults.nextRequired} />
          </Field>
        </div>

        <div className="grid grid-cols-3 gap-4">
          <Field label="必要来院ペース">
            <select name="requiredVisitInterval" className="input" defaultValue={defaults.requiredVisitInterval ?? ""}>
          ...
          <Field label="回復度(健康度幸福度)">
            <select name="healthHappinessScore" className="input" defaultValue={defaults.healthHappinessScore ?? ""}>
              <option value="">未設定</option>
          ...
          {showRank ? (
            <Field label="ランク">
              <select name="rank" className="input" defaultValue={defaults.rank ?? ""}>
            ...
            <p className="text-xs text-stone-400 self-end pb-2">
              ランクは最新の来院記録でのみ編集できます
            </p>
          )}
        </div>
        ...
        <label className="flex items-center gap-1.5 text-sm text-stone-700">
          <input
            type="checkbox"
            name="isManualReturnFlag"
            value="true"
            defaultChecked={defaults.isManualReturnFlag}
        ...
        <Field label="口コミ・紹介">
          <div className="flex flex-col gap-2 rounded-md border border-stone-200 bg-stone-50 p-3">
            <div className="flex flex-wrap items-center gap-2 text-sm text-stone-700">
              <input
                type="checkbox"
                id="testimonialObtained"
                name="testimonialObtained"
                value="true"
                defaultChecked={defaults.testimonialObtained}
              ...
              <span className="hidden items-center gap-1.5 text-stone-500 peer-checked:flex">
                取得日:
                <input type="date" name="testimonialObtainedDate" className="input py-1" defaultValue={defaults.testimonialObtainedDate} />
              </span>
            </div>
            <div className="flex flex-wrap items-center gap-2 text-sm text-stone-700">
              <input
                ...
                name="referralGiven"
                value="true"
                defaultChecked={defaults.referralGiven}
                className="peer accent-emerald-800"
              />
              <label htmlFor="referralGiven">紹介をしてくれた</label>
              <span className="hidden items-center gap-1.5 text-stone-500 peer-checked:flex">
                人数:
                <input type="number" name="referralCount" min={1} defaultValue={defaults.referralCount ?? 1} className="input py-1 w-20" />
              </span>
            </div>
          </div>
        </Field>
...
function TagCheckboxes({ name, options, defaultValues = [] }: { name: string; options: readonly string[]; defaultValues?: string[] }) {
  return (
