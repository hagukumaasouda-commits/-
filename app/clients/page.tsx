...
export default async function ClientsPage({
  ...
  const clients = await prisma.client.findMany({
    where: q
      ? { OR: [{ name: { contains: q } }, { kana: { contains: q } }, { externalCustomerNo: { contains: q } }] }
      : undefined,
    include: {
          ...
          <tbody>
            {clients.map((c) => (
              <tr key={c.id} className="border-b border-stone-100 last:border-0 hover:bg-stone-50">
                ...
                <td className="px-4 py-2 text-stone-600">{c.externalCustomerNo ?? "—"}</td>
                ...
                <td className="px-4 py-2 text-right tabular-nums">{c.initialVisitCount + c._count.visits}</td>
