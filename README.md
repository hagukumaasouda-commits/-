...
`http://localhost:3000` で以下にアクセスできます。
...
- **ホットペッパービューティーの予約連携**: 一般に外部連携用の公開APIが提供されていないため、CSV取り込み・通知メール解析・手動入力のいずれで運用するか要確認。現状 `Reservation.source` に `HOTPEPPER` を用意していますが、投入経路は未実装(手動作成のみ)です。
- **LINE公式アカウントのMessaging API(自動リマインド)**: チャネルアクセストークンの発行・友だち追加時のユーザーID連携が必要。`LINE_CHANNEL_ACCESS_TOKEN` の環境変数だけ用意していますが、送信処理自体は未実装です。
- **認証・アクセス制御**: 現時点でログイン機能はありません。既往歴・症状などの要配慮個人情報を扱うため、本番運用前にスタッフ認証と権限設計(誰が全顧客を見られるか)を追加することを強く推奨します。
